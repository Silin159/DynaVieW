...
#SBATCH -J finetune-dynaview-vinabench
#SBATCH -t 60:00:00
#SBATCH --output=finetune-dynaview-vinabench.out
#SBATCH --error=finetune-dynaview-vinabench.err
#SBATCH --nodes 8
    ...
    if [ "$NODE" == "$MASTER_NODE" ]; then
        srun \
          ...
          torchrun --nnodes=$NODE_NUM --node_rank=0 --nproc_per_node=4 --master_addr=$MASTER_ADDR --master_port=29500 \
            run_bagel_ft.py \
            --dataset_config_file $PROJECT_ROOT_AT/modeling/data/configs/vinabench.yaml \
            ...
            --vwm_attn True \
            --vit_attn_trans True \
            --results_dir finetune-dynaview-vinabench \
            --checkpoint_dir $OUT_DIR \
            --wandb_api_key $WANDB_API_KEY \
            --wandb_project state-transition-vwm \
            --wandb_name finetune-dynaview-vinabench &
    else
        ((NODE_COUNT++))
        srun \
          --nodes=1 --ntasks=1 -w $NODE \
