...
class TrainingArguments:
    ...
    wandb_api_key: str = field(
        default="",
        metadata={"help": "Weights & Biases API Key."}
    )
    ...
    ce_loss_reweighting: bool = field(
        default=True,
        metadata={"help": "Reweight CE loss by token importance (provided via ce_loss_weights), i.e., applying schema token re-weighted CE loss."}
    )
    ...
    img_hist_dropout: float = field(
        default=0.3,
        metadata={"help": "Attention dropout of history states (images)"}
    )
    vit_attn_trans: bool = field(
        default=True,
        metadata={"help": "Whether ViT images pay attention to previous transitions"}
    )
    vwm_attn: bool = field(
        default=True,
        metadata={"help": "Whether applying multimodal selective attention for state-transition modeling"}
    )
...
def main():
    ...
    train_dataset = PackedDataset(
        ...
        img_hist_dropout=training_args.img_hist_dropout,
        vit_attn_trans=training_args.vit_attn_trans,
        vwm_attn=training_args.vwm_attn,
    )
    ...
    for curr_step, data in enumerate(train_loader, start=train_step):
        ...
        if ce is not None:
            ...
            loss_dict["ce"] = ce.detach() * training_args.ce_weight
        ...
        if mse is not None:
            ...
            loss_dict["mse"] = mse.detach() * training_args.mse_weight
