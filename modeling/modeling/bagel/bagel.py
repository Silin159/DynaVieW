...
class Bagel(PreTrainedModel):
    ...
    def forward(
        ...
        if self.config.visual_gen:
            ...
            for latent, (h, w) in zip(padded_latent, patchified_vae_latent_shapes):
                ...
                latent = latent.transpose(0, 1).transpose(1, 3).transpose(3, 4).reshape(-1, p * p * self.latent_channel)
        ...
        mse = None
        if self.config.visual_gen and mse_loss_indexes.any():
    ...
    def prepare_prompts(self, curr_kvlens, curr_rope, curr_stsegs, prompts, tokenizer, new_token_ids, vwm_mask, add_special, img_dropout=0.0, in_type="prompt"):
        ...
        curr = 0
        newlens, new_rope, newsegs = list(), list(), list()
        assert len(prompts) == 1
        for prompt, curr_kvlen, curr_position_id, st_seg in zip(prompts, curr_kvlens, curr_rope, curr_stsegs):
            ...
            elif add_special == "start":
                text_ids = [new_token_ids['bos_token_id']] + text_ids
            elif add_special == "end":
                text_ids = text_ids + [new_token_ids['eos_token_id']]
            txt_len = len(text_ids)
            text_token_lens.append(txt_len)
            packed_text_ids.extend(text_ids)
            packed_text_position_ids.extend(range(curr_position_id, curr_position_id + txt_len))
            packed_text_indexes.extend(range(curr, curr + txt_len))
            
            if vwm_mask:
                prev_seg_len = sum([seg[1] for seg in st_seg])
                attention_mask = torch.zeros((txt_len, prev_seg_len+txt_len), dtype=torch.bool)
                curr_mask = 0

                last_img_id = 0
                for sid, seg in enumerate(st_seg):
                    if seg[0] == "image":
                        last_img_id = sid

                for sid, seg in enumerate(st_seg):
                    if seg[0] == "prompt":
                        attention_mask[:, curr_mask:curr_mask+seg[1]] = 1
                    elif seg[0] == "image":
                        if sid == last_img_id or random.random() >= img_dropout:
                            attention_mask[:, curr_mask:curr_mask+seg[1]] = 1
                    elif seg[0] == "schema":
                        pass
                    else:
                        raise NotImplementedError(f"{seg[0]} is not suppoprted")
                    curr_mask += seg[1]
                attention_mask[:, prev_seg_len:prev_seg_len+txt_len] = torch.ones((txt_len, txt_len)).tril()
                attention_mask = torch.zeros_like(attention_mask, dtype=torch.float).masked_fill_(~attention_mask, float("-inf"))
            else:
                attention_mask = None

            newlens.append(curr_kvlen + txt_len)
            new_rope.append(curr_position_id + txt_len)
            curr += txt_len
            st_seg.append([in_type, txt_len])
            newsegs.append(st_seg)
        ...
        return generation_input, newlens, new_rope, newsegs
    ...
    def forward_cache_update_text(
        ...
        extra_inputs = {}
        if self.use_moe:
            extra_inputs = {"mode": "und", "attention_mask": attention_mask}
    ...
    def prepare_vit_images(self, curr_kvlens, curr_rope, curr_stsegs, images, transforms, new_token_ids, vwm_mask, img_dropout=0.0):
        ...
        newlens, new_rope, newsegs = list(), list(), list()
        assert len(images) == 1
        for image, curr_kvlen, curr_position_id, st_seg in zip(images, curr_kvlens, curr_rope, curr_stsegs):
            packed_key_value_indexes.extend(range(curr, curr + curr_kvlen))
            curr += curr_kvlen
            ...
            if vwm_mask:
                prev_seg_len = sum([seg[1] for seg in st_seg])
                attention_mask = torch.zeros((num_img_tokens+2, prev_seg_len+num_img_tokens+2), dtype=torch.bool)
                curr_mask = 0
                ...
                for sid, seg in enumerate(st_seg):
                    if sid < attn_start:
                        if seg[0] == "prompt":
                            attention_mask[:, curr_mask:curr_mask+seg[1]] = 1
                        elif seg[0] == "image" and random.random() >= img_dropout:
                            attention_mask[:, curr_mask:curr_mask+seg[1]] = 1
                    elif sid == attn_start:
                        assert seg[0] == "image" or attn_start == 0
                        attention_mask[:, curr_mask:curr_mask+seg[1]] = 1
                    else:
                        assert (seg[0] == "prompt" or seg[0] == "schema")
                        attention_mask[:, curr_mask:curr_mask+seg[1]] = 1
                    curr_mask += seg[1]

                attention_mask[:, prev_seg_len:prev_seg_len+num_img_tokens+2] = torch.ones((num_img_tokens+2, num_img_tokens+2))
                attention_mask = torch.zeros_like(attention_mask, dtype=torch.float).masked_fill_(~attention_mask, float("-inf"))
            else:
                attention_mask = None
            ...
            st_seg.append(["image", num_img_tokens+2])
            newsegs.append(st_seg)
    ...
    def prepare_vae_images(self, curr_kvlens, curr_rope, curr_stsegs, images, transforms, new_token_ids, timestep=0, vwm_mask=False, img_dropout=0.0):
    ...
    def prepare_vae_latent(self, curr_kvlens, curr_rope, curr_stsegs, image_sizes, new_token_ids, vwm_mask, img_dropout=0.0):
        packed_text_ids, packed_text_indexes = list(), list()
        ...
        query_curr = curr = 0
        assert len(curr_kvlens) == 1
        for (H, W), curr_kvlen, curr_position_id, st_seg in zip(image_sizes, curr_kvlens, curr_rope, curr_stsegs):
            packed_key_value_indexes.extend(range(curr, curr + curr_kvlen))
            curr += curr_kvlen
            ...
            if vwm_mask:
                prev_seg_len = sum([seg[1] for seg in st_seg])
                attention_mask = torch.zeros((num_image_tokens+2, prev_seg_len+num_image_tokens+2), dtype=torch.bool)
                curr_mask = 0
                ...
                attention_mask[:, prev_seg_len:prev_seg_len+num_image_tokens+2] = torch.ones((num_image_tokens+2, num_image_tokens+2))
                attention_mask = torch.zeros_like(attention_mask, dtype=torch.float).masked_fill_(~attention_mask, float("-inf"))
            else:
                attention_mask = None
    ...
    def prepare_vae_latent_cfg(self, curr_kvlens, curr_rope, curr_stsegs, image_sizes, vwm_mask, attn_ctx, img_dropout=0.0):
        ...
        for (H, W), curr_kvlen, curr_position_id, st_seg in zip(image_sizes, curr_kvlens, curr_rope, curr_stsegs):
            ...
            if vwm_mask:
                ...
                if attn_start == 0 and st_seg[0][0] != "image":
                    for seg in st_seg[:-attn_ctx]:
                        curr_mask += seg[1]
                    for seg in st_seg[-attn_ctx:]:
                        assert (seg[0] == "prompt" or seg[0] == "schema")
                        attention_mask[:, curr_mask:curr_mask+seg[1]] = 1
        ...
        generation_input = {
            ...
            "cfg_attention_mask": attention_mask,
        }
    ...
    def generate_image(
        ...
        cfg_text_attention_mask: Optional[torch.Tensor] = None,
        # cfg_img
        ...
        cfg_img_attention_mask: Optional[torch.Tensor] = None,
        ...
        for i, t in tqdm(enumerate(timesteps), total=len(timesteps)):
            ...
            v_t = self._forward_flow(
                ...
                cfg_text_attention_mask=cfg_text_attention_mask,
                # cfg_img
                ...
                cfg_img_attention_mask=cfg_img_attention_mask,
                cfg_type=cfg_type,
                # cache
    ...
    def _forward_flow(
        ...
        if cfg_text_scale > 1.0:
            extra_inputs["attention_mask"] = cfg_text_attention_mask
            if self.language_model.model.enable_taylorseer:
        ...
        if cfg_img_scale > 1.0:
            extra_inputs["attention_mask"] = cfg_img_attention_mask
            if self.language_model.model.enable_taylorseer:
    ...
    def prepare_start_tokens(self, curr_kvlens, curr_rope, curr_stsegs, new_token_ids, vwm_mask, complete, img_dropout=0.0):
        packed_start_tokens, packed_key_value_indexes = list(), list()
        packed_query_position_ids = list()

        curr = 0
        assert len(curr_kvlens) == 1
        for curr_kvlen, curr_position_id, st_seg in zip(curr_kvlens, curr_rope, curr_stsegs):
            packed_key_value_indexes.extend(range(curr, curr + curr_kvlen))
            if complete:
                packed_start_tokens.append(new_token_ids['space_token'])
            else:
            ...
            if vwm_mask:
                prev_seg_len = sum([seg[1] for seg in st_seg])
                attention_mask = torch.zeros((1, prev_seg_len), dtype=torch.bool)
                curr_mask = 0
    ...
    def generate_text(
        ...
        while step < max_length:
            ...
            if attention_mask is not None:
                attention_mask = torch.cat([attention_mask, torch.zeros((1, 1), dtype=torch.float)], dim=1)
