...
class PackedAttention(Qwen2Attention):
    ...
    def forward_inference(
        ...
        if attention_mask is None:
            ...
            query_states = packed_query_states.transpose(0, 1)
            key_states = merged_key_states[:, :, None, :].repeat(1, 1, self.num_key_value_groups, 1)
            key_states = key_states.reshape(-1, self.num_heads, self.head_dim).transpose(0, 1)
            value_states = merged_value_states[:, :, None, :].repeat(1, 1, self.num_key_value_groups, 1)
            value_states = value_states.reshape(-1, self.num_heads, self.head_dim).transpose(0, 1)
            attn_output = scaled_dot_product_attention(
            ...
            packed_attn_output = attn_output.squeeze(0).transpose(0, 1).reshape(-1, self.hidden_size)
...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def forward_inference(
        ...
        if self.use_moe:
            ...
            extra_inputs.update(attention_mask=attention_mask)
