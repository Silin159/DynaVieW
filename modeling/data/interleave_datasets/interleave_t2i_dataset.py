...
from ..data_utils import parse_json_schema
...
class InterleavedBaseIterableDataset(DistributedIterableDataset):
    ...
    def _add_text(self, data, text, need_loss, enable_cfg=True, schema_ce_reweight=False, vwm=False, prompt=False):
        text_ids = self.tokenizer.encode(text)
        data['num_tokens'] += len(text_ids)
        data['text_ids_list'].append(text_ids)
        if vwm:
            data_type = "text_trans"
        elif prompt:
        ...
        if schema_ce_reweight:
            text_schema = parse_json_schema(text)
            if text_schema is not None:
                schema_ids = self.tokenizer.encode(str(text_schema))
                data['schema_ids_list'].append(schema_ids)
            else:
                data['schema_ids_list'].append(None)
        else:
            data['schema_ids_list'].append(None)
        return data

    def _add_image(self, data, image, need_loss, need_vae, need_vit, enable_cfg=True, vwm=False):
        assert need_loss or need_vae or need_vit

        if need_loss:
            if vwm:
                data_type = "vae_image_state"
            else:
                data_type = "vae_image"
            data['sequence_plan'].append(
                {
...
class JSONLStandardIterableDataset(DistributedIterableDataset):
    ...
    def __init__(
        ...
        shuffle_lines=False, shuffle_seed=0, ablation="",
    ):
        """
        jsonl_path_list: list of jsonl file paths
        data_dir_list: list of data directories
        ...
        self.data_paths = self.get_data_paths(jsonl_path_list, data_dir_list, num_used_data, shuffle_lines, shuffle_seed)
        self.set_epoch()
    ...
    def __iter__(self):
        ...
        while True:
            ...
            for row_idx, (data, image_dir) in enumerate(data_paths_per_worker_, start=row_start_id):
                try:
                    row = json.loads(data)
                    data = self.parse_row(row, self.ablation)
                    if len(data) == 0:
                        continue
                    data['data_indexes'] = {
                    ...
                    print(f'Error {e} in row#{row_start_id}')
                    continue
                yield data
