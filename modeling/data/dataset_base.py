...
class PackedDataset(torch.utils.data.IterableDataset):
    def __init__(
        ...
        self.img_hist_dropout = img_hist_dropout
        self.vit_attn_trans = vit_attn_trans
        self.vwm_attn = vwm_attn
        ...
        while True:
            ...
            if sequence_status['curr'] + num_tokens > self.max_num_tokens:
                if len(buffer) < self.max_buffer_size and not sample_from_buffer:
                    ...
                    print(f"Drop and Yield data with length {sum(sequence_status['sample_lens'])} and batch {len(batch_data_indexes)}")
            ...
            if sequence_status['curr'] >= self.expected_num_tokens:
                print(f"Yield data with length {sum(sequence_status['sample_lens'])} and batch {len(batch_data_indexes)}")
                data = self.to_tensor(sequence_status)
                data['batch_data_indexes'] = batch_data_indexes
                yield data
                sequence_status = self.set_sequence_status()
    ...
    def pack_sequence(self, sample, sequence_status):
        ...
        schema_ids_list = sample['schema_ids_list']
        ...
        for item in sequence_plan:
            ...
            if 'text' in item['type']:
                ...
                if len(schema_ids_list) > 0:
                    schema_ids = schema_ids_list.pop(0)
                else:
                    schema_ids = None
                ...
                if item['loss'] == 1:
                    ...
                    sequence_status['ce_loss_weights'].extend(schema2weight(text_ids, schema=schema_ids))
                ...
                if 'trans' in item['type']:
                    attn_modes.append("causal_trans")
                elif 'prompt' in item['type']:
                    attn_modes.append("causal_prompt")
                else:
                    attn_modes.append("causal")
                sequence_status['packed_position_ids'].extend(range(curr_rope_id, curr_rope_id + curr_split_len))
                curr_rope_id += curr_split_len

            elif 'vit_image' in item['type']:
                ...
                # update sequence status
                if 'state' in item['type']:
                    attn_modes.append("full_state")
                else:
                    attn_modes.append("full")
                sequence_status['packed_position_ids'].extend([curr_rope_id] * curr_split_len)
                curr_rope_id += 1

            elif 'vae_image' in item['type']:
                image_tensor = image_tensor_list.pop(0)
        ...
        if not self.use_flex:
            ...
            sequence_status['nested_attention_masks'].append(prepare_attention_mask_vwm(split_lens, attn_modes, img_hist_dropout=self.img_hist_dropout,
                                                                                        vit_attn_trans=self.vit_attn_trans, vwm_attn=self.vwm_attn))
        else:
